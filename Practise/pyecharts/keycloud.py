...
import Practise.set_pro.count_pro as counter

content = "I am happy to join with you today in what will go down in history as the greatest demonstration for " \
...
words = []
for key, val in counter.getWordCount(content).items():
    words.append((key, val))


def getWordCloud(wordlist):
    return WordCloud().add("", wordlist, word_size_range=[20, 100]).set_global_opts(
        title_opts=opts.TitleOpts(title="马丁路德金演讲稿词云"))


getWordCloud(words).render('wordcloud.html')
